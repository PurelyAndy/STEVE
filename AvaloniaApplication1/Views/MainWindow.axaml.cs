...
public partial class MainWindow : Window
{
    ...
    private void RollBingo()
    {
        ...
        List<(string Text, double Size)> bingoItems =
        [
        ...
        for (int i = 0; i < bingoItems.Count; i++)
        {
            int item1 = i;
            int item2 = Random.Shared.Next(i, bingoItems.Count);
            (bingoItems[item1], bingoItems[item2]) = (bingoItems[item2], bingoItems[item1]);
        }
        ...
        for (int j = 1; j <= 5; j++)
        {
            ...
            string[] split = bingoItems[(i - 1) * 5 + j - 1].Text.Split("<i>");
            ...
            Border border = new()
            {
                ...
                Child = new TextBlock
                {
                    FontSize = bingoItems[(i - 1) * 5 + j - 1].Size,
